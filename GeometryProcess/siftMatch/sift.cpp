// sift.cpp: implementation of the sift class.
//
//////////////////////////////////////////////////////////////////////
...
IplImage* create_init_img( float *BlockImg, int Width,int Height,float maxgray, int img_dbl, double sigma );
IplImage* create_init_imgblock( float *BlockImg, int Width,int Height,int img_dbl, double sigma );
IplImage* convert_to_gray32( IplImage* );
IplImage* convert_to_gray32( float *BlockImg, int Width,int Height,float maxgray );
IplImage* convert_to_gray32block( float *BlockImg, int Width,int Height );
...
@see _sift_features()
*/
int sift_features(float *BlockImg, int Width,int Height,float maxgray,struct feature** feat )
{
	return _sift_features( BlockImg, Width,Height,maxgray, feat, SIFT_INTVLS, SIFT_SIGMA, SIFT_CONTR_THR,
							SIFT_CURV_THR, SIFT_IMG_DBL, SIFT_DESCR_WIDTH,
							SIFT_DESCR_HIST_BINS );

}
int sift_featuresblock(float *BlockImg, int Width,int Height,struct feature** feat )
{
	return _sift_featuresblock( BlockImg, Width,Height, feat, SIFT_INTVLS, SIFT_SIGMA, SIFT_CONTR_THR,
							SIFT_CURV_THR, SIFT_IMG_DBL, SIFT_DESCR_WIDTH,
							SIFT_DESCR_HIST_BINS );
...
@see sift_keypoints()
*/
int _sift_features( float *BlockImg, int Width,int Height,float maxgray,struct feature** feat, int intvls,
				   ...
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	...
	init_img = create_init_img( BlockImg, Width,Height,maxgray, img_dbl, sigma );
	octvs = int(log( double(MIN( init_img->width, init_img->height )) ) / log(2.) - 2);
...
int _sift_featuresblock( float *BlockImg, int Width,int Height,struct feature** feat, int intvls,
				   double sigma, double contr_thr, int curv_thr,
				   int img_dbl, int descr_width, int descr_hist_bins )
{
	IplImage* init_img;
	...
	/* build scale space pyramid; smallest dimension of top level is ~4 pixels */
	init_img = create_init_imgblock( BlockImg, Width,Height, img_dbl, sigma );
	octvs = int(log( double(MIN( init_img->width, init_img->height )) ) / log(2.) - 2);
	gauss_pyr = build_gauss_pyr( init_img, octvs, intvls, sigma );
	dog_pyr = build_dog_pyr( gauss_pyr, octvs, intvls );
...
IplImage* create_init_img( float *BlockImg, int Width,int Height,float maxgray, int img_dbl, double sigma )
{
	...
	gray = convert_to_gray32( BlockImg, Width,Height,maxgray );
	if( img_dbl )
	{
		sig_diff = sqrt( sigma * sigma - SIFT_INIT_SIGMA * SIFT_INIT_SIGMA * 4 );
		dbl = cvCreateImage( cvSize( Width*2, Height*2 ),
			IPL_DEPTH_32F, 1 );
		cvResize( gray, dbl, CV_INTER_CUBIC );
		cvSmooth( dbl, dbl, CV_GAUSSIAN, 0, 0, sig_diff, sig_diff );
		cvReleaseImage( &gray );
...
IplImage* create_init_imgblock( float *BlockImg, int Width,int Height, int img_dbl, double sigma )
{
	...
	gray = convert_to_gray32block( BlockImg, Width,Height );
//	cvShowImage("debug",gray);
	if( img_dbl )
	{
		...
		cvResize( gray, dbl, CV_INTER_CUBIC );
		cvSmooth( dbl, dbl, CV_GAUSSIAN, 0, 0, sig_diff, sig_diff );
		gray->imageData = gray->imageDataOrigin = NULL;
		cvReleaseImage( &gray );
		return dbl;
	}
	else
...
IplImage* convert_to_gray32( float *BlockImg, int Width,int Height,float maxgray )
{
	...
	gray32->nSize=112;         /* sizeof(IplImage) */
    gray32->ID=0;            /* version (=0)*/
    gray32->nChannels=1;     /* Most of OpenCV functions support 1,2,3 or 4 channels */
    gray32->alphaChannel=0;  /* ignored by OpenCV */
    gray32->depth=IPL_DEPTH_32F;         /* pixel depth in bits: IPL_DEPTH_8U, IPL_DEPTH_8S, IPL_DEPTH_16S,
                           IPL_DEPTH_32S, IPL_DEPTH_32F and IPL_DEPTH_64F are supported */

	strcpy(gray32->colorModel,"GRAY");    
	strcpy(gray32->channelSeq,"GRAY");  /* ditto */    
    gray32->dataOrder=0;     /* 0 - interleaved color channels, 1 - separate color channels.
                           cvCreateImage can only create interleaved images */
    gray32->origin=0;        /* 0 - top-left origin,
                           1 - bottom-left origin (Windows bitmaps style) */
    gray32->align=4;         /* Alignment of image rows (4 or 8).
                           OpenCV ignores it and uses widthStep instead */
	gray32->width=Width;         /* image width in pixels */
    gray32->height=Height;        /* image height in pixels */
    
	gray32->roi=NULL;   //struct _IplROI *roi;/* image ROI. if NULL, the whole image is selected */	
    gray32->maskROI=NULL; /* must be NULL */
    gray32->imageId=NULL;;     /* ditto */
    gray32->tileInfo=NULL; /* ditto */       
	
    memset(gray32->BorderMode,0,4*sizeof(int)); /* ignored by OpenCV */
    memset(gray32->BorderConst,0,4*sizeof(int)); /* ditto */

	gray32->widthStep = (((gray32->width * gray32->nChannels *
		(gray32->depth & ~IPL_DEPTH_SIGN) + 7)/8)+ gray32->align - 1) & (~(gray32->align - 1));
    
    gray32->imageSize = gray32->widthStep * gray32->height;	

	
	gray32->imageData=(char *)malloc(gray32->height*gray32->widthStep);//new float[gray32->height*gray32->width];  /* pointer to aligned image data */
	gray32->imageDataOrigin=gray32->imageData;  /* pointer to aligned image data */
    
	for(int i=0;i<gray32->height;i++)
	{
		for(int j=0;j<gray32->width;j++)
		{
			((float *)gray32->imageData)[i*gray32->width+j]=BlockImg[i*gray32->width+j]/maxgray;		
		}
	}
...
//	CvMat* orig = &cvMat( Height,Width , CV_32FC1, BlockImg );

	gray32 = cvCreateImage(/*cvGetSize(orig)*/cvSize(Width,Height),IPL_DEPTH_32F,1);
//	float maxGray = BlockImg[0];
	for(int i=0;i<gray32->height;i++)
	{
		for(int j=0;j<gray32->width;j++)
		{
//			maxGray = maxGray>BlockImg[i*gray32->width+j]?maxGray:BlockImg[i*gray32->width+j];
			((float *)gray32->imageData)[i*gray32->width+j]=BlockImg[i*gray32->width+j];		
		}
	}
	//for(int i=0;i<gray32->height;i++)
	//{
	...
	//	}
	//}
//	cvGetImage(orig,gray32);

//	cvReleaseImage(&gray32);
	//gray32->nSize=Width*Height;//112;         /* sizeof(IplImage) */
 //   gray32->ID=0;            /* version (=0)*/
 //   gray32->nChannels=1;     /* Most of OpenCV functions support 1,2,3 or 4 channels */
 //   gray32->alphaChannel=0;  /* ignored by OpenCV */
 //   gray32->depth=IPL_DEPTH_32F;         /* pixel depth in bits: IPL_DEPTH_8U, IPL_DEPTH_8S, IPL_DEPTH_16S,
 ...
 //   
	...
	//float t;
	//int i,j;
	//for( i=0;i<gray32->height;i++)
	//{
	//	for( j=0;j<gray32->width;j++)
	//	{
	//		t=BlockImg[i*gray32->width+j];
	//		if(t>maxgray)
	//		{
	//			maxgray=t;
	//		}
...
// 	a.height=Height;
// 	a.width=Width;
// 	gray32=cvCreateImage( a, IPL_DEPTH_32F, 1 );
// 	
// //	gray32=new IplImage;	
// //	
// //	gray32->nSize=112;         /* sizeof(IplImage) */
...
// //    gray32->imageSize = gray32->widthStep * gray32->height;	
// //
// //	//20101210 add *sizeof(float)
// //	gray32->imageData=(char *)malloc(gray32->height*gray32->widthStep*sizeof(float));//new float[gray32->height*gray32->width];  /* pointer to aligned image data */
// //	gray32->imageDataOrigin=gray32->imageData;  /* pointer to aligned image data */
// //    
// 	float maxgray=0;
// 	float t;
...
// 		}
// 	}
// 	memset(gray32->imageData,0,gray32->imageSize);
// 	for(i=0;i<gray32->height;i++)
// 	{
// 		for(j=0;j<gray32->width;j++)
// 		{
// 			((float *)gray32->imageData)[i*gray32->width+j]=BlockImg[i*Width+j]/maxgray;		
// 		}
// 	}
// 	return gray32;
// }
						   ...
						   CvMemStorage* storage )
{
	...
	for( o = 0; o < octvs; o++ )
	{
		for( i = 1; i <= intvls; i++ )
		{
			for(r = SIFT_IMG_BORDER; r < dog_pyr[o][0]->height-SIFT_IMG_BORDER; r+=step)
			{
				for(c = SIFT_IMG_BORDER; c < dog_pyr[o][0]->width-SIFT_IMG_BORDER; c+=step)
				{
